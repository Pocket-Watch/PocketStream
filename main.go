...
func main() {
	...
	defer func() {
		if err := recover(); err != nil {
			...
			LogError(&args, "PocketStream CRASHED: %v\n\n%s", err, buffer[:size])
		}
	}()
	...
	strings.TrimSuffix(args.Destination, "/")
...
func handleStreaming(args *Arguments, cmd *exec.Cmd) {
	if err := os.MkdirAll(args.OutputDirectory, 0o755); err != nil {
		LogError(args, "Failed to create output directory: %v", err)
		os.Exit(1)
	}
...
func uploadRequest(args *Arguments, path string, attempt int) {
	...
	if !fileExists(path) {
		LogError(args, "File at %v doesn't exist!", path)
		return
	}
	data, err := os.ReadFile(path)
	if err != nil {
		LogError(args, "Failed to read chunk file generated by FFMPEG: %v", err)
		return
	}
	...
	if err != nil {
		LogError(args, "Failed to create a POST request: %v", err)
		panic(err)
	}
	...
	if err != nil {
		LogError(args, "Failed to send data to the server: %v", err)
...
func executeCommandStdPipe(args *Arguments, cmd *exec.Cmd) {
	...
	if err := cmd.Wait(); err != nil {
		LogError(args, "Failed to start FFMPEG: %v", err)
	}
}
...
func checkPortClaimedPeriodically(args *Arguments, interval time.Duration, attempts int) {
	...
	for range attempts {
		...
		if err != nil {
			LogError(args, "TCPv4 port check failed due to: %v", err)
			os.Exit(1)
		}
		...
		if err != nil {
			LogError(args, "TCPv6 port check failed due to: %v", err)
			os.Exit(1)
		}

		if len(socketsV6) > 0 || len(socketsV4) > 0 {
			fmt.Printf("Confirmed server listening at %v after %v\n", address, time.Since(start).String())
			return
		}
	...
	LogError(args, "Failed to determine whether address %v is claimed after %v attempts", address, attempts)
}

func startStream(args *Arguments) {
	req, err := http.NewRequest("POST", args.Destination+StreamStartEndpoint, nil)
	if err != nil {
		LogError(args, "Failed to start stream. POST request to the server failed: %v", err)
		os.Exit(1)
	}
	...
	if err != nil {
		LogError(args, "Failed to start stream. Server refused the connection: %v", err)
		os.Exit(1)
	}
	...
	if resp.StatusCode != 200 {
		LogError(args, "Server responded with status code: %v", resp.Status)
